...
dbCon.connect(function (err) {
    ...
    const saltRounds = 10;
    const myPlaintextPassword = 'pass'; // replace with plaintext password chosen by you OR retain the same value
    const passwordHash = bcrypt.hashSync(myPlaintextPassword, saltRounds);

    const rowToBeInserted = {
        acc_name: 'moha0950',            // replace with acc_name chosen by you OR retain the same value
        acc_login: 'moha0950',           // replace with acc_login chosen by you OR retain the same value
